...
('Oxygen_Common_Auth_Role_Bot','Bot','robot'),
('Oxygen_Common_Auth_Role_BotAudit','Bot (Audit)','robot'),
...
('Oxygen_Common_Auth_Role_ScopeBot','Scope Bot','robot'),
('Oxygen_Common_Auth_Role_ScopeBotAudit','Scope Bot (Audit)','robot');
...
create table auth_users (
    ...
    must_change tinyint(1) not null
)  engine InnoDB;
insert into auth_users values
(1,'admin','Oxygen', 'Admin', 'admin@example.com', 1, lower(sha1(concat('admin',':salt'))), 1, 1, unix_timestamp(now()), unix_timestamp(now()), lower(concat(sha(concat('P@ssw0rd', 'salt')), hex('salt'))), 1);
